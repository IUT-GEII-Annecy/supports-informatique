...
\begin{UPSTIinfor}{La division en langage C}
    \label{info:division}
    En C, l'opérateur \texttt{/} ne se comporte pas de la même façon selon le type des deux valeurs qu'il divise.
    \begin{description}
        \item[Si les deux opérandes sont des \textbf{entiers} :] la division est une \textbf{division entière} (division euclidienne) : le résultat ne conserve que le quotient, la partie décimale est perdue. Par exemple, \verb|17 / 5| vaut \verb|3| (et non \texttt{3,4}).
        \item[Si l'un des deux opérandes est \textbf{flottant} (\texttt{float} ou \texttt{double}) :] la division est une division réelle. Par exemple, \verb|17.0 / 5| vaut \verb|3.4|.
    \end{description}
\end{UPSTIinfor}

\begin{UPSTIinfor}{Le modulo \% : le reste de la division euclidienne}
    \label{info:modulo}
    L'opérateur \texttt{\%} renvoie le \textbf{reste} de la division entière entre deux entiers. Par exemple, \verb|17 % 5| vaut \verb|2|, car $17 = 3\times5+2$.

    \textbf{Attention :} \texttt{\%} ne s'utilise qu'entre \textbf{entiers} ; on ne peut pas l'utiliser directement avec un \texttt{float} ou un \texttt{double}.
\end{UPSTIinfor}
