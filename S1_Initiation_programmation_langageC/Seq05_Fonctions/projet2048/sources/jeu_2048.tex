...
\begin{UPSTIinfor}{Passage de tableau en paramètre}
    Ici, à chaque fois que nous passons un paramètre de type \texttt{Grid}, il s'agit en fait d'un tableau. Lorsque l'on passe un tableau en paramètre d'une fonction, le comportement est différent que lorsque l'on passe une simple variable. En effet, \textbf{TOUTES les modifications faites au tableau à l'intérieur de la fonction se répercutent dans le contexte ayant appelé ladite fonction.} Cela vient du fait que l'on passe en fait un pointeur vers le tableau. Cette notion n'est pas déveoppée ici. 
\end{UPSTIinfor}
