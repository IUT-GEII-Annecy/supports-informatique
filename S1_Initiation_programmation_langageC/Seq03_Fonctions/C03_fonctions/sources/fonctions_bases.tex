...
Des durées, données en minutes, doivent être converties au format « heures et minutes » (XXhXXmin) par un programme. Sans fonction, on pourrait écrire :
...
\end{lstlisting}
  \begin{description}
    \item[type de retour :] type de la valeur renvoyée par la fonction (\texttt{int}, \texttt{float}…). S'il n'y a rien à renvoyer, on indique \texttt{void} ;
    \item[nom de la fonction :] identifiant qui permet d'appeler la fonction ;
    \item[paramètres :] variables reçues en entrée, entre parenthèses, séparées par des virgules. S'il n'y a aucun paramètre, on indique \texttt{void} ;
...
int main(void) {
    ...
    if (aire1 > aire2) {
        printf("Le premier rectangle a la plus grande aire (%i)\n", aire1);
    } else {
        printf("Le second rectangle a la plus grande aire (%i)\n", aire2);
    }
...
Beaucoup de fonctions (la majorité) effectuent des opérations en vue de \textit{renvoyer} un résultat. Le mot-clef \texttt{return} sert alors à retourner ce résultat vers la fonction appelante. Cependant, il existe également des fonctions qui ne renvoient pas de résultat. On peut citer en exemples : \textit{Affichage d'une valeur} ou l'\textit{Initialisation d'un système}.
...
\begin{UPSTIwarning}{Absence de résultat et absence de paramètre}
...
void afficher_aire(int largeur, int longueur) {
    ...
    printf("Aire du rectangle %ix%i : %i\n", largeur, longueur, aire);
    return; // Fin de l'exécution de la fonction sans valeur retournée.
}
\end{lstlisting}

\texttt{afficher\_aire} ne renvoie rien (elle affiche directement) : c'est pour cela qu'elle est déclarée \texttt{void}, et qu'elle ne contient pas de \texttt{return valeur;}. Notons qu'elle \emph{utilise} la fonction \texttt{aire\_rectangle} qui lui renvoie la valeur de l'aire. \texttt{afficher\_aire} stocke alors cette valeur dans la variable \texttt{aire} avant de l'utiliser dans l'affichage. 

\begin{UPSTIactivite}[][Avec ou sans retour ?]
On souhaite manipuler des températures.
\UPSTIquestion{Écrire la définition de la fonction de prototype \texttt{float celsius\_vers\_fahrenheit(float celsius)} qui renvoie la température convertie, sachant que $F = C \times 9/5 + 32$.}
\UPSTIquadrillage{2}
\UPSTIquestion{Écrire la définition de la fonction de prototype \texttt{void afficher\_fahrenheit(float celsius)} qui affiche la phrase \texttt{"X degres Celsius valent Y degres Fahrenheit"}, en réutilisant la fonction précédente.}
\UPSTIquadrillage{2}
\end{UPSTIactivite}
...
\begin{UPSTIwarning}{Une variable locale ne survit pas à l'appel}
  ...
  Aussi, sa valeur aura disparu au prochain appel de cette fonction et elle ne pourra donc pas être réutilisée. 
\end{UPSTIwarning}
...
\end{lstlisting}
  ...
  \texttt{
    Dans doubler, n vaut 10
    Dans main, x vaut toujours 5
  }
  \texttt{x} n'a jamais été modifiée, seule la copie \texttt{n} l'a été.
\end{UPSTIwarning}

\subsection{Ce qu'il se passe en mémoire}
\UPSTIquadrillage{7.5}