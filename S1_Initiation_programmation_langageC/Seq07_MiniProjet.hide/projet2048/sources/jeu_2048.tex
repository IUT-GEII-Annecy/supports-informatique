\section{Présentation du projet 2048}
\begin{center}
...
\begin{itemize}
    \item Le jeu se joue sur une grille 4x4.
    \item Au début du jeu, deux tuiles (2 ou 4) sont placées aléatoirement sur la grille.
    \item A chaque tour, le joueur déplace les tuiles dans l'une des quatre directions (haut, bas, gauche, droite). Toutes les tuiles se déplacent alors \textbf{aussi loin que possible} dans la direction choisie.
    \item Lorsque deux tuiles \textbf{identiques} se touchent lors d'un déplacement, elles fusionnent en une seule tuile avec une valeur égale à la somme des deux tuiles.
    \item Après chaque déplacement, une nouvelle tuile (2 ou 4) est ajoutée à une position vide de la grille.
    \item Le jeu se termine lorsque le joueur atteint la tuile 2048 (victoire) ou lorsque la grille est pleine et qu'aucun mouvement n'est possible (défaite).
\end{itemize}

\UPSTIetape{Dans un premier temps, vous pouvez prendre 5 min pour jouer à 2048 en ligne afin de bien comprendre les règles du jeu. Par exemple, vous pouvez jouer ici : \url{https://play2048.co/}}
