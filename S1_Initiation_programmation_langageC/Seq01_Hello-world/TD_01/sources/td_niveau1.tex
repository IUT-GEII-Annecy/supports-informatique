...
\begin{UPSTIexemples}[La variable : une case étiquetée]{}
    Une variable, comme une variable Scratch, c'est une \textbf{case mémoire}, portant une \textbf{étiquette} (son nom) et contenant une \textbf{valeur} qui peut changer au cours du programme. En C, cette case a en plus un \textbf{type} fixé une fois pour toutes, qui indique la nature de ce qu'elle contient.
...
else
{
    ...
    \end{lstlisting}
    \UPSTIquestion{Traduire ce programme en pseudo-code (avec \texttt{SI}, \texttt{SINON SI}, \texttt{SINON}, \texttt{FIN SI} et \texttt{AFFICHER}).}
