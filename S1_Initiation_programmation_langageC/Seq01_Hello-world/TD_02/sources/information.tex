\begin{UPSTIinfor}{Rappel : la structure \texttt{switch case}}
    \label{info:switch}
...
switch (expression) {
    ...
    \begin{itemize}
        ...
        \item Le bloc \texttt{default} est facultatif. Il n'a \textbf{pas besoin d'être placé en dernier} : le \texttt{switch} teste les \texttt{case} dans l'ordre du code, et \texttt{default} n'est choisi que si aucune valeur ne correspond, où qu'il soit écrit. En pratique, on le place presque toujours en dernier, par lisibilité --- et surtout, s'il n'est pas en dernier, il lui faut son propre \texttt{break} pour ne pas \og tomber \fg dans le \texttt{case} suivant.
    \end{itemize}
