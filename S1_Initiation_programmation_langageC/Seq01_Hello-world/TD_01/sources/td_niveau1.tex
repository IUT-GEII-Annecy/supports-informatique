...
\begin{UPSTIexercice}{Trace d'exécution : la caisse d'un distributeur}
    Un distributeur de boissons exécute les lignes suivantes lorsqu'un client insère des pièces.
    \begin{lstlisting}[language=C]
int credit = 0;
int prix = 90;
int monnaie;
...
credit = 0;              // (4) la machine rend la monnaie, le credit repart a 0
    \end{lstlisting}
    ...
    \begin{center}
        \begin{tabular}{|c|c|c|c|}
            \hline
            \textbf{Ligne} & \texttt{credit} & \texttt{prix} & \texttt{monnaie} \\
            \hline
            avant (1)      & 0               & 90            & ?                \\
            \hline
            (1)            &                 &               &                  \\[0.5cm]
        ...
        \end{tabular}
    \end{center}
    \UPSTIquestion{Quelle est la valeur finale de \texttt{monnaie} ? A-t-elle changé après la ligne (4) ?}
\end{UPSTIexercice}
...
\begin{UPSTIprofOnlyEnv}
    \begin{UPSTIcorrectionP}{Trace d'exécution : la caisse d'un distributeur}
        \begin{center}
            ...
            \end{tabular}
        \end{center}
        \texttt{monnaie} vaut \textbf{10} et ne change plus après la ligne (3) : la ligne (4) ne modifie que \texttt{credit}, chaque variable étant une case indépendante des autres.
    \end{UPSTIcorrectionP}
\end{UPSTIprofOnlyEnv}
