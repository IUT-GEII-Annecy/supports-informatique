...
\end{lstlisting}
    \UPSTIquestion{Compléter le tableau de trace ci-dessous, ligne par tour de boucle, jusqu'à la sortie de la boucle.}
...
\end{lstlisting}
    ...
    \UPSTIquestion{Quelle est la toute dernière valeur de \texttt{i} pour laquelle le bloc de la boucle s'exécute encore ?}
\end{UPSTIexercice}
...
Boucle terminee
\end{lstlisting}
        La dernière valeur pour laquelle le bloc s'exécute est \texttt{i = 1} (car \texttt{1 > 0} est vrai) ; ensuite \texttt{i} devient \texttt{-2}, et \texttt{-2 > 0} est faux, donc la boucle s'arrête sans afficher \texttt{i = -2}.
    \end{UPSTIcorrectionP}
\end{UPSTIprofOnlyEnv}
