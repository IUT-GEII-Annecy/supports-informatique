...
\begin{UPSTIinfor}{Rôle d'un fichier d'en-tête (A retenir !)}
  ...
  Chaque prototype est souvent accompagné d’un \textbf{commentaire} de documentation.
\end{UPSTIinfor}
...
On souhaite créer un module \texttt{conversions} regroupant deux fonctions de prototype \texttt{float km\_vers\_miles(float km)} et \texttt{float miles\_vers\_km(float miles)}.
...
Jusqu'ici, vos programmes n'occupaient qu'un fichier \texttt{.c}, par exemple \texttt{programme.c}, que vous avez pu compiler avec la commande \texttt{make programme}. La commande \texttt{make} fait ici appel au \emph{compilateur C}, nommé \texttt{gcc}, et exécute la commande suivante :

\begin{lstlisting}[language=bash,caption={Compilation d'un seul fichier source directement avec gcc}]
...
L'option \texttt{-o} sert à choisir le nom qu'aura le fichier exécutable.
